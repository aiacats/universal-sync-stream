...
mod internal {
    ...
    fn raw_fcc_to_frameformat(raw: OSType) -> Option<FrameFormat> {
        match raw {
            ...
            // BGRA format (used by OBS Virtual Camera and other virtual cameras)
            kCVPixelFormatType_32BGRA => Some(FrameFormat::BGRA),
            _ => None,
        }
    }
    ...
    impl AVCaptureDevice {
        ...
        pub fn set_all(&mut self, descriptor: CameraFormat) -> Result<(), NokhwaError> {
            ...
            // Store first available format as fallback (for virtual cameras like OBS)
            ...
            for format in format_list {
                ...
                // Store the first format as fallback
                    ...
                    && dimensions.width == descriptor.resolution().width() as i32
                {
                    ...
                    // If we found the resolution but not the frame rate, use first available range
            ...
            if selected_format.is_null() && !fallback_format.is_null() {
    ...
    impl AVCaptureVideoDataOutput {
        ...
        pub fn set_frame_format(&self, format: FrameFormat) -> Result<(), NokhwaError> {
            let cmpixelfmt = match format {
                ...
                FrameFormat::BGRA => kCVPixelFormatType_32BGRA,
            };
